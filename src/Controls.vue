    ...
    <div v-if="playerSize !== 'hide'" :class="playerControlsPosition === 'fixed' ? 'fixed mb-18 sm:mb-0' : ''"
        ...
        <div class="flex justify-center items-center">
            ...
            <div v-if="playerSize === 'minimal'" @click="stop"
                 ...
                 class="trans-fast mx-3 flex items-center cursor-pointer">
            ...
            <!-- play-->
            <div v-if="!options.playing" @click="togglePlayPause"
                 :class="(theme.active === 'dark' ? 'opacity-70' : '') + (playerSize === 'minimal' ? ' mx-3' : ' mx-2')"
            ...
            <div v-if="options.playing" @click="togglePlayPause"
                 :class="(theme.active === 'dark' ? 'opacity-70' : '') + (playerSize === 'minimal' ? ' mx-3' : ' mx-2')"
                 class="trans-fast flex items-center justify-center w-8 h-8 cursor-pointer">
                <svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect x="12" y="0.545471" width="7.63636" height="22.9091" rx="1.90909"
                          :fill="theme[theme.active].color"/>
                    <rect y="0.545471" width="7.63636" height="22.9091" rx="1.90909" :fill="theme[theme.active].color"/>
                </svg>
            </div>

            <!--  speed-->
            <div v-if="playerSize === 'standard'" @click="setPlayerSpeed" class="mx-2 flex items-center cursor-pointer">
                <span
                    class="min-w-8 bg-white border border-grey-light px-1 py-1 rounded-md text-grey-darkest text-xs text-center font-lf">{{ speeds[activeSpeedIndex] }}x</span>
            </div>

            <seeker class="mx-2" v-if="playerSize === 'standard'" :theme="theme" :options="options"></seeker>

            <!-- loop-->
            <div @click="toggleLoop"
                 :class="(options.loop === false ? 'opacity-50' : '')+' '+(theme.active === 'dark' && options.loop === true ? 'opacity-70' : '') + ( playerSize === 'minimal' ? ' mx-3' : ' mx-2' )"
                ...
                <svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        ...
                        :fill="theme[theme.active].color" :stroke="theme[theme.active].color" stroke-width="0.262318"/>
                    <path
                        ...
                        :fill="theme[theme.active].color" :stoke="theme[theme.active].color" stroke-width="0.262318"/>
                    ...
                    <path
                        ...
                        :fill="theme[theme.active].color" mask="url(#path-3-outside-1)"/>
                </svg>
            </div>
